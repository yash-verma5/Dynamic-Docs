...
```bash
INIT=$(node ./.gemini/get-shit-done/bin/gsd-tools.cjs state load)
...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.cjs commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
